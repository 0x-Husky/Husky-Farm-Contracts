...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accHSKDPerShare;   // Accumulated HSKDs per share, times PRECISION_FACTOR. See below.
    }

    // The HSKD TOKEN!
    IBEP20 public huskydex;
    ...
    // The block number when HSKD mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 HSKDReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHSKDPerShare = accHSKDPerShare.add(HSKDReward.mul(PRECISION_FACTOR).div(lpSupply));
        }
        return user.amount.mul(accHSKDPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 HSKDReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accHSKDPerShare = pool.accHSKDPerShare.add(HSKDReward.mul(PRECISION_FACTOR).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Stake huskydex tokens to SmartChef
    function deposit(uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHSKDPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHSKDPerShare).div(PRECISION_FACTOR);


        emit Deposit(msg.sender, _amount);
    }

    // Withdraw huskydex tokens from STAKING.
